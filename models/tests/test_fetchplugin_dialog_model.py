...
class TestFetchPluginDialogModel(unittest.TestCase):
    ...
    def local_plugin_url(cls, plugin_name):
        ...
        return urllib.pathname2url(TestFetchPluginDialogModel.local_plugin(plugin_name))

    @classmethod
    def plugin_url_params(cls, plugin_name):
        """Returns the URL to the specified plugin name when served by the test server"""
        return 'http://localhost:{0}/{1}'.format(cls.PORT, TestFetchPluginDialogModel.local_plugin_url(plugin_name))
    ...
    def good_plugin_url(self):
        ...
        return TestFetchPluginDialogModel.plugin_url_params('good_medfilter_plugin.zip')
