...
class Tukey(UltrasonicGate):
    ...
    def left_lobe(self):
        ...
        for point in range(lower_limit, int(upper_limit)):
            fpoint = float(point)
            left_lobe_array.append(.5*(1+math.cos(math.pi*(2*fpoint/(self.alpha*(self.num_points-1))-1))))
    ...
    def right_lobe(self):
        ...
        for point in range(int(lower_limit), int(upper_limit)):
            fpoint = float(point)
            right_lobe_array.append(.5*(1+math.cos(math.pi*(2*fpoint/(self.alpha*(self.num_points-1))-2/self.alpha+1))))
